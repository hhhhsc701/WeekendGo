TOOL_DEFINITIONS = [
    ...
    },
    {
        ...
        "function": {
            "name": "search_poi",
            "description": "搜索附近的POI景点、餐厅、博物馆、酒店等",
            "parameters": {
                "type": "object",
                "properties": {
                    "query": {"type": "string", "description": "搜索关键词，如'景点'、'餐厅'、'酒店'"},
        ...
        "function": {
            ...
            "parameters": {
                ...
                "properties": {
                    "city": {
                        "type": "string",
                        "description": "最终目的地城市。用户未输入目的地时必须填写模型随机选择的具体城市。",
                    },
                    ...
                    "items": {
                        ...
                        "description": "行程项目列表。每个项目必须尽量包含地点坐标和本步骤预算；必须包含酒店入住/休息或退房相关安排。",
                    },
...
1a. 如果用户没有输入目的地城市，先根据出发城市、天数、预算、兴趣、同行和备注随机选择一个明确目的地城市；后续所有工具调用都围绕这个城市，不要生成“多个备选城市”。
2. 调用geocode获取城市坐标
3. 调用get_weather获取天气
4. 多次调用search_poi搜索不同类型地点（景点、餐厅、博物馆、酒店等）
5. 如有出发城市，调用query_trains查询火车（仅国内）
6. 收集足够信息后，立即调用finish输出行程

工具使用规则：
- geocode: 根据城市名判断，中文名用amap，英文名用google
- search_poi: 可多次调用搜索不同类型，location参数用geocode返回的坐标；必须至少搜索一次酒店/住宿相关POI
- finish: 必须在收集天气+POI后调用，输出完整行程JSON
- finish.city: 用户未输入目的地时必须填写你最终选择的具体目的地城市；标题和所有地点也必须与该城市一致。
...
- finish.items: 必须包含酒店。多日行程至少包含酒店入住/休息和退房相关安排；单日行程也要包含酒店/住宿推荐或入住安排。酒店place.category填"酒店"，estimated_cost填写当晚或该安排费用。
